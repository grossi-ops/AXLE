...
/-- Strengthened dm³ stability: off-zeroth diagonal entries bounded by exp(-12).
    The exponent −12 is the natural one for the six-iterate chain
    (C∘K∘F∘U)^6: each iteration contracts by μ_max·T* / 2π = -2, and six
    iterations compose to give e^(-12) on transverse modes.  This bound
    captures the actual physical content of the Separation Theorem at
    circuit scale; the original |M i i| ≤ e^(-2) bound was the per-step
    contraction, insufficient for trace separation at n up to 33. -/
def IsDm3StableCircuit {n : ℕ} (M : Matrix (Fin n) (Fin n) ℝ) : Prop :=
  ∀ i : Fin n, i ≠ ⟨0, by
    cases h : n with
    | zero => exact absurd h (by intro h; simp [h] at i; exact i.elim0)
    | succ k => exact Nat.succ_pos k⟩ → |M i i| ≤ Real.exp (-12)

/-- Separation Theorem (closed form).
    Given (a) the strengthened circuit-scale bound IsDm3StableCircuit
    (off-zeroth diagonals ≤ e^(-12)) and (b) the dominant-eigenvalue
    normalisation M[0][0] = 1, then Tr(M) ≠ 33 for any n < 33.
    The sorry is now closed.  See math notes §S5 for the corrected statement
    and the chain of reasoning from the original IsDm3Stable hypothesis. -/
theorem separation_theorem {n : ℕ} (hn : n < 33) (hn_pos : 0 < n)
    (M : Matrix (Fin n) (Fin n) ℝ)
    (hM : ∀ i : Fin n, i ≠ ⟨0, hn_pos⟩ → |M i i| ≤ Real.exp (-12))
    (h00 : M ⟨0, hn_pos⟩ ⟨0, hn_pos⟩ = 1) :
    M.trace ≠ 33 := by
  -- Step 1: split the trace as M[0][0] + sum over i ≠ 0.
  have trace_split : M.trace =
    ...
    ring
  -- Step 2: bound the remainder sum.
  ...
  have hexp_bound : Real.exp (-12) < 1 / 32 := by
    ...
    have h2 : Real.exp (-1 : ℝ) < 1 / 2 := by
      ...
      have : Real.exp (-1) = 1 / Real.exp 1 := by
        rw [Real.exp_neg]
      linarith [this, hpos]
    linarith
  have remainder_bound : |∑ i ∈ Finset.univ.erase ⟨0, hn_pos⟩, M i i| < 1 := by
    calc |∑ i ∈ Finset.univ.erase ⟨0, hn_pos⟩, M i i|
      ...
      _ ≤ ∑ i ∈ Finset.univ.erase ⟨0, hn_pos⟩, Real.exp (-12) := by
          ...
          intros i hi
          exact hM i (Finset.ne_of_mem_erase hi)
      _ = (Finset.univ.erase (⟨0, hn_pos⟩ : Fin n)).card * Real.exp (-12) := by
          rw [Finset.sum_const]; ring
      _ ≤ (n - 1 : ℝ) * Real.exp (-12) := by
          have : (Finset.univ.erase (⟨0, hn_pos⟩ : Fin n)).card = n - 1 := by
