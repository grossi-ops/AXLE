...
`N(0, σ²)` Gaussian centered at 0. Neither matches the actual dm³ toy model:
  ...
  · the real stationary density is a Gaussian **centered at r = 1** with a
    coefficient of 4, coming from the drift `F ≈ −4(r − 1)`.
...
verified by `lake build` before being trusted.
  ...
  Fix 3 — Stationary density:
    ...
    new:  ρ(x) ∝ exp(−4(x − 1)² / σ²)   centered at 1, coefficient 4  (from drift −4(r−1))
...
/-! ## Fix 3: Stationary Density — centered at r = 1, coefficient 4

The dm³ stationary density is the invariant measure of
`dX_t = F(X_t) dt + σ dW_t` with drift `F(r) ≈ −4(r − 1)` near the attractor
`r = 1`. The Ornstein–Uhlenbeck stationary law is Gaussian with

    mean = 1,   variance = σ² / (2·4) = σ² / 8,

so the density is `ρ(x) ∝ exp(−4(x − 1)² / σ²)`. The old file used
`exp(−x²/σ²)`: **wrong center (0 vs 1), wrong coefficient (1 vs 4), wrong drift.** -/
...
/-- **Corrected stationary density.**
    ...
    `ρ(x) = (2 / (σ·√π)) · exp(−4(x − 1)² / σ²)`

    Gaussian with mean 1 and variance σ²/8; normalising constant
    `2/(σ√π) = 1 / ∫ exp(−4(x−1)²/σ²) dx`. -/
noncomputable def ρ (x : ℝ) : ℝ :=
  (2 / (σ * Real.sqrt Real.pi)) * Real.exp (-(4 * (x - 1) ^ 2 / σ ^ 2))

/-- The density is peaked at the attractor r = 1: the exponent vanishes there. -/
theorem ρ_peak_at_one : (4 * ((1 : ℝ) - 1) ^ 2 / σ ^ 2) = 0 := by ring
...
theorem ρ_normalised (hσ : 0 < σ) : ∫ x : ℝ, ρ σ x = 1 := by
  sorry
  -- Route: substitute y = 2(x−1)/σ so ∫ exp(−4(x−1)²/σ²) dx = (σ/2)·∫ exp(−y²) dy
...
/-- **Fokker–Planck stationarity** of ρ for the dm³ OU model
    `dX = −4(X−1)dt + σ dW`. Expected to stay open until Mathlib has SDE
...
theorem ρ_is_stationary_of_dm3_flow : True := by trivial
  ...
  -- generator L*ρ = 0 for drift −4(x−1), diffusion σ²/2.
